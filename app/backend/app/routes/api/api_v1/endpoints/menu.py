...
async def my_menu(request: Request):
...
async def user_menu(userid: str):
...
async def my_menu_delete(request: Request):
    user = await get_current_user(request.headers['Authorization'].split(' ')[1])
    if (menu := await db.get_collection("menu").find_one({"user_id": user.id})) is not None:
        await db.get_collection("menu").delete_one({"user_id": user.id})
        return HTTPException(
            status_code=201, detail=f"menu for user {user.id} deleted")
    raise HTTPException(
...
async def add_food_to_menu(request: Request, food: List[Food] = Body(...)):
    try:
...
async def delete_food_from_menu(request: Request, food_id: str):
    try:
        menu = await my_menu(request)
    except HTTPException as e:
        raise HTTPException(
